        ...
        const float f_type)
{
    ...
    // filter features
    filterFeatures(f_imageNoise, f_lambdaWeight, f_precisionThreshold);
...
float* CSfop::return_kp()
{
    float* kp = new float[m_features_p->size()*2 + 1];
    int counter = 1;
    ...
    for (std::list<CFeature>::const_iterator
        ...
        kp[counter] = l_iter->x ;
        kp[counter+1] = l_iter->y ;
        counter += 2;
    }
    kp[0] = counter; // This stores the size of float array 
    return kp;
