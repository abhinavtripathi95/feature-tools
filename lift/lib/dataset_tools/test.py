...
from lift.lib.custom_types import pathConfig
from lift.lib.dataset_tools.helper import load_patches
from lift.lib.dump_tools import loadh5
from lift.lib.kp_tools import IDX_ANGLE, loadKpListFromTxt
...
class data_obj(object):
    ...
    def load_data(self, param, image_file_name, kp_list):
        ...
        cur_data = self.load_data_for_set(
            pathconf, param, image_file_name, kp_list)
        self.x = cur_data[0]
        self.y = cur_data[1]
        self.ID = cur_data[2]
        self.pos = cur_data[3]
    ...
    def load_data_for_set(self, pathconf, param,
        ...
        # kp = np.asarray(loadKpListFromTxt('results/img1_kp.txt'))
